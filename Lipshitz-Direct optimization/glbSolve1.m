...
syms k1 k2 q z;
intloss=matlabFunction(int((z-k1)*normpdf(z,0,1),z,k1,inf)); %定义缺货积分函数
f=(A1*DD1+A2*DD2)/q+(DD1*h1+DD2*h2)*q/2/(DD1+DD2)+(k1*h1*sigma1+k2*h2*sigma2)*sqrt(aa/q+bb*q+cc)+(lc1*sigma1*intloss(k1)+lc2*sigma2*intloss(k2))*(DD1+DD2)*sqrt(aa/q+bb*q+cc)/q; %定义目标函数
...
%   h  = tomsol(11,xx,yy); % conhull is an internal subfunction
   ...
   for jj = 1:length(S)  
      ...
      for ii = 1:length(I) 
         i = I(ii);
         e_i=[zeros(i-1,1);zeros(n-i,1)]; %%根据求解变量个数修改
         c_m1=C(:,j)+delta;
%        c_m1 = C(:,j);
         c_m1(i)=c_m1(i)+delta;
         x_m1 = x_L+c_m1.*x_D; 
